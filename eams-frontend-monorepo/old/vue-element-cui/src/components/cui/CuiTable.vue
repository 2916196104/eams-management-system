<template>
  <div class="table-builder">
    ...
    <el-row style="margin-bottom: 10px" :gutter="10">
      <el-col :span="20">
        <template v-for="(item,index) in topActions">
          <el-button :key="index" :type="item.type || ''" v-if="handleMultiButtonVisible(item)" @click="item.click(multipleSelectionVal)" :icon="item.icon" size="mini">{{item.title}}
          </el-button>
        </template>
        <span>&nbsp;</span>
      </el-col>
    ...
    <!--  -->
    <el-table id="printTable" class="cui-table bueatyScroll" border :data="data" v-loading="loading" ref="tableBuilder" :max-height="height" :default-expand-all="expand" highlight-current-row size='small' :empty-text="emptyText" @sort-change="handleSortChange" @selection-change="handleSelectionChange" @row-click="handleRowClick" :row-class-name="rowClassName" :show-summary="showSummary" :summary-method="()=>sums" style="margin-bottom: 10px; width:100%" :span-method='objectSpanMethod'>
      <el-table-column type="expand" v-if="expand">
        <template slot-scope="scope">
          <slot name="expandContent" :data="scope.row"></slot>
        </template>
      </el-table-column>
      <el-table-column type="selection" align="center" prop='selectable' :selectable="selectFunc" v-if="selectable" width="45">
      </el-table-column>
      <el-table-column label="序号" type="index" prop='indexed' :index="indexMethod" v-if="indexed"></el-table-column>
      <el-table-column v-for="(item,index) in columnFields" :key="index" :label="item.title" :prop="item.name" :sortable="item.sortable || false" :align="item.align || 'left'" :width="item.width||''" v-if="typeof item.visible != 'undefined' ? item.visible(item) : true">
        <template slot-scope="scope">
          <editalbe-text v-if="item.editable" :field="item.name" :row="scope.row" :editFunc="item.editFunc" :editConf="item.editConf">
      ...
      <!--            fixed="right"-->
      <el-table-column label="操作" :width="actionWidth" prop='actionButtons' class-name="action-column" v-if="actions && actions.length > 0">
        <template slot-scope="scope">
              ...
              <el-dropdown v-if="item.type === 'list'">
                ...
                <el-dropdown-menu slot="dropdown">
                  <template v-for="(opt,ind) in item.options">
                    <el-dropdown-item :key="ind" v-if="handleButtonVisible(opt, scope.row)" @click.native="opt.click && opt.click(scope.row)">{{opt.title}}</el-dropdown-item>
                  </template>
                </el-dropdown-menu>
              </el-dropdown>
              <template v-else>
                <el-button :type="item.type||'text'" :class="item.class||''" @click.stop="item.click && item.click(scope.row)"  v-if="handleButtonVisible(item, scope.row)" :icon="item.icon" :size="item.size||'small'">
                  {{item.title}}
                </el-button>
              </template>
            </el-tooltip>
    ...
    <el-row class="fixed-bottom">
      <el-col :span="(topActions && topActions.length>0) ? 12 : 0">
      </el-col>
      <!-- v-if="this.showPaginate && pagination.pageCount > 1" -->
      <el-col :span="(topActions && topActions.length>0) ? 12 : 24" v-if="this.showPaginate">
        <el-pagination small background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="params.page" :page-sizes="[ 30, 50, 100, 200]" :page-size="params.pageSize" :layout="paginateLayout" :total="pagination.total">
        </el-pagination>
      </el-col>
    </el-row>
  </div>
    ...
    spanFlitterData(fieldName) {
      ...
      this.data.forEach((item, index) => {
        if (index === 0) {
          ...
          if (item[fieldName] === this.data[index - 1][fieldName]) { //第一列需合并相同内容的判断条件
      ...
      for(var f of this.config.fields) {
        for(var l of this.checkedLabels) {
          if(f.title == l) {
            ...
            f1.visible = function(data) {return true}
            res.push(f1)
          }
        }
      }
