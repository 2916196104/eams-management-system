...
// 预导入图标
const preloadicons = ['i-carbon:settings', 'i-carbon:application-virtual', 'i-carbon:assembly', 'i-carbon:favorite', 'i-carbon:user']
...
<template>
  ...
  <view class="box-border">
    <!-- 系统通知 -->
    <wd-notice-bar
      :text="textArray" :delay="3" direction="vertical" type="warning" custom-class="space"
    ...
    >
      ...
      <template #suffix>
        <div class="text-16px text-#4d80f0">
          查看
        </div>
      </template>
    </wd-notice-bar>
    <!-- 轮播图 -->
    <wd-swiper
      v-model:current="current" height="200px" class="mt-3" :list="swiperList" autoplay
