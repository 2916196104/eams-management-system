  ...
  <section class="calendar-container">
    ...
    <template v-else-if="viewMode === 'week'">
      <section class="week-view-header">
        ...
        >
          {{ weekdays[idx] }}<br>{{ formatWeekDay(day) }}
        </span>
      </section>
...
function handleDayCellClick(hour: number) {
  const d = new Date(viewCursor.value.getFullYear(), viewCursor.value.getMonth(), viewCursor.value.getDate(), hour, 0, 0, 0)
  selectedDate.value = new Date(viewCursor.value.getFullYear(), viewCursor.value.getMonth(), viewCursor.value.getDate())
  emit('date-select', selectedDate.value)
