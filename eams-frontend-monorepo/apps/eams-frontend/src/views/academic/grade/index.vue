  ...
  <div class="grade-page">
    ...
    <el-dialog v-model="upgradeDialogVisible" title="年级升级" width="400px">
      ...
      <div class="upgrade-tip">
        <p>即将升级以下年级：</p>
        <ul>
          <li v-for="item in selectedGrades" :key="item.id">
            {{ item.name }} → {{ getUpgradedName(item.name) }}
          </li>
        </ul>
      </div>
      <template #footer>
...
import { getGradeList, saveGrade, deleteGrade, upgradeGrade, type GradeItem, type GradeListParams, type SaveGradeParams } from '@/apis/academic/grade';
