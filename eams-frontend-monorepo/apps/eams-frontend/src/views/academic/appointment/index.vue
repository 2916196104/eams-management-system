  ...
  <div class="appointment-page">
    ...
    <div v-if="viewMode === 'list'">
      ...
      <el-card class="action-card">
        ...
        <el-button type="warning" :disabled="selectedIds.length === 0" @click="handleBatchReschedule">批量调课</el-button>
      </el-card>
      ...
      <el-card>
        ...
        <el-table :data="tableData" v-loading="loading" border stripe @selection-change="handleSelectionChange">
          ...
          <el-table-column v-if="visibleColumns.includes('studentName')" prop="studentName" label="预约人" min-width="120" />
          <el-table-column v-if="visibleColumns.includes('studentMobile')" prop="studentMobile" label="手机号" width="130" />
          <el-table-column v-if="visibleColumns.includes('lessonTitle')" prop="lessonTitle" label="预约课程" min-width="150" />
          <el-table-column v-if="visibleColumns.includes('lessonTime')" prop="lessonTime" label="上课时间" width="160" />
      ...
      <el-card>
        ...
        <div class="calendar-header">
          <div class="calendar-nav">
            <el-button @click="prevPeriod">上一{{ calendarView === 'month' ? '月' : calendarView === 'week' ? '周' : '天' }}</el-button>
            <span class="current-date">{{ currentDateText }}</span>
            <el-button @click="nextPeriod">下一{{ calendarView === 'month' ? '月' : calendarView === 'week' ? '周' : '天' }}</el-button>
...
import { getAppointmentList, getAppointmentCalendar, approveAppointment, cancelAppointment, getAppointmentDetail, type AppointmentItem, type AppointmentListParams, type CalendarParams, type AppointmentCalendarVo } from '@/apis/academic/appointment';
...
const handleRefresh = () => { if (viewMode.value === 'list') loadListData(); else loadCalendarData(); };
...
const handleSearch = () => { pagination.pageIndex = 1; loadListData(); };
const handleReset = () => {
  Object.assign(searchForm, { studentName: '', courseName: '', dateRange: [], verifyState: '' });
  handleSearch();
};
const handleSelectionChange = (rows: AppointmentItem[]) => { selectedIds.value = rows.map(row => row.id); };
const handleSizeChange = (size: number) => { pagination.pageSize = size; loadListData(); };
const handleCurrentChange = (page: number) => { pagination.pageIndex = page; loadListData(); };
...
const confirmReschedule = async () => {
  if (!rescheduleForm.newTime) { ElMessage.warning('请选择新的上课时间'); return; }
  ...
  } catch { ElMessage.error('调课失败'); } finally { rescheduleLoading.value = false; }
};
...
const getAppointmentsByDateTime = (date: string, hour: number) => calendarData.value.filter(item => item.lessonTime?.startsWith(date) && parseInt(item.lessonTime?.split(' ')[1]?.split(':')[0] || '0') === hour);
...
const handleDateClick = (data: any) => { selectedDate.value = data.day; };
const handleTimeSlotClick = (date: string, hour: number) => { console.log('点击时间格子:', date, hour); };
const handleCalendarViewChange = () => { loadCalendarData(); };
const prevPeriod = () => {
...
const goToday = () => { calendarDate.value = new Date(); selectedDate.value = dayjs().format('YYYY-MM-DD'); loadCalendarData(); };
const goToListView = () => { viewMode.value = 'list'; };
...
.appointment-page {
  ...
  .calendar-header {
    ...
    .current-date { font-size: 18px; font-weight: bold; min-width: 200px; text-align: center; }
  }
  ...
  .month-view .calendar-cell {
    ...
    .appointments {
      ...
      .appointment-item {
        ...
        white-space: nowrap;
        cursor: pointer;
        &.pending { background-color: #fff3e0; color: #e6a23c; }
        &.confirmed { background-color: #f0f9eb; color: #67c23a; }
        &.cancelled { background-color: #fef0f0; color: #f56c6c; text-decoration: line-through; }
        &:hover { opacity: 0.8; }
      }
    }
  }
  ...
  .week-view {
    overflow-x: auto;
    .week-header { display: flex; border-bottom: 1px solid #ebeef5; }
    .week-day-header { flex: 1; text-align: center; padding: 12px 0; border-right: 1px solid #ebeef5; }
    .week-body { display: flex; }
    .time-column { width: 60px; flex-shrink: 0; .time-slot { height: 60px; border-bottom: 1px solid #ebeef5; text-align: center; font-size: 12px; line-height: 60px; } }
    .day-column { flex: 1; border-right: 1px solid #ebeef5; .hour-slot { height: 60px; border-bottom: 1px solid #ebeef5; position: relative; cursor: pointer; &:hover { background-color: #f5f7fa; } } }
    .appointment-block { position: absolute; left: 2px; right: 2px; background-color: #f0f9eb; color: #67c23a; font-size: 10px; padding: 2px 4px; border-radius: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; cursor: pointer; }
  }

  .day-view {
    .day-header { text-align: center; padding: 12px; border-bottom: 1px solid #ebeef5; .day-date { font-size: 16px; font-weight: bold; } }
    .day-body { display: flex; }
    .time-column { width: 60px; flex-shrink: 0; .time-slot { height: 60px; border-bottom: 1px solid #ebeef5; text-align: center; font-size: 12px; line-height: 60px; } }
    .day-content { flex: 1; .hour-slot { height: 60px; border-bottom: 1px solid #ebeef5; position: relative; cursor: pointer; &:hover { background-color: #f5f7fa; } } }
    .appointment-block { position: absolute; left: 2px; right: 2px; background-color: #f0f9eb; color: #67c23a; font-size: 10px; padding: 2px 4px; border-radius: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; cursor: pointer; }
  }
...
@media print {
  .search-card, .action-card, .table-toolbar, .pagination-wrapper, .calendar-header, .daily-list-card { display: none; }
}
</style>
