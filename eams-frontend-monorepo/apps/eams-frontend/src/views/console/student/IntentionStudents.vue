<template>
	<section class="intention-students-page">
		<!-- 筛选区 -->
		...
		<section class="toolbar-panel">
			<div class="toolbar-left compact-actions">
				...
				<el-tooltip content="导入意向学员" placement="top">
...
async function handleSwitchToStudying() {
	...
	try {
		await ElMessageBox.confirm(`确认将选中学员从意向转为在学？（共 ${ids.length} 人）`, "转为在学", {
			type: "warning",
		});
...
const intentionImportVisible = ref(false);
const importTitle = "导入意向学员";
function handleOpenImport() {
	intentionImportVisible.value = true;
}
function handleImportSuccess() {
	intentionImportVisible.value = false;
	loadList();
}
...
function isLikelyMockLoremMessage(text: string): boolean {
	...
	return /\b(lorem|ipsum|dolor|fugiat|veniam|irure|laborum|proident|laboris|nisi|consectetur|adipiscing|commodo|eiusmod|dolore|magna|mollit|nostrud|nulla|pariatur|ullamco|excepteur|culpa|elit|quis|sunt|tempor|ut|velit|officia|deserunt|anim|esse|cillum|eu|incididunt|minim|occaecat|reprehenderit|sint|duis|aute)\b/i.test(
		t,
	);
}
...
async function loadList() {
	...
	try {
		...
		if (requestSeq !== latestRequestSeq.value) return;
		ElMessage.error(e?.message || "加载意向学员失败");
		tableRows.value = [];
		total.value = 0;
	} finally {
...
function handleExport() {
	...
	try {
		...
		downloadTextFile(`意向学员导出_${Date.now()}.csv`, csv);
