...
class ClassStudentItemDTO : public oatpp::DTO {
    ...
    DTO_FIELD(UInt64, id);
    DTO_FIELD_INFO(id) {
        info->description = ZH_WORDS_GETTER("stu.field.id");
    }

    // 学员姓名
    DTO_FIELD(String, name);
    DTO_FIELD_INFO(name) {
        info->description = ZH_WORDS_GETTER("stu.field.studentName");
    }

    // 性别 (0未知, 1男, 2女)
    DTO_FIELD(Int32, gender);
    DTO_FIELD_INFO(gender) {
        info->description = ZH_WORDS_GETTER("stu.field.gender");
    }

    // 剩余课时数
    DTO_FIELD(Int32, remainingLessonCount);
    DTO_FIELD_INFO(remainingLessonCount) {
        info->description = ZH_WORDS_GETTER("stu.field.remainingCount");
    }
};
...
class StudentDetailDTO : public oatpp::DTO {
    ...
    DTO_FIELD_INFO(mobile) {
        info->description = ZH_WORDS_GETTER("stu.field.mobile");
    }
    ...
    DTO_FIELD(Int32, age);
    DTO_FIELD_INFO(age) {
        info->description = ZH_WORDS_GETTER("stu.field.age");
    }

    // 生日
    DTO_FIELD(String, birthday);
    DTO_FIELD_INFO(birthday) {
        info->description = ZH_WORDS_GETTER("stu.field.birthday");
    }

    // 学员类型
    DTO_FIELD(Int32, stage);
    DTO_FIELD_INFO(stage) {
        info->description = ZH_WORDS_GETTER("stu.field.stage");
    }

    // 备注
    DTO_FIELD(String, remark);
    DTO_FIELD_INFO(remark) {
        info->description = ZH_WORDS_GETTER("stu.field.remark");
    }
};
...
class StudentCourseItemDTO : public oatpp::DTO {
    ...
    DTO_FIELD(UInt64, id);
    DTO_FIELD_INFO(id) {
        info->description = ZH_WORDS_GETTER("stu.field.recordId");
    }

    // 课程名称
    DTO_FIELD(String, courseName);
    DTO_FIELD_INFO(courseName) {
        info->description = ZH_WORDS_GETTER("stu.field.courseName");
    }

    // 已上课次
    DTO_FIELD(Int32, countLessonComplete);
    DTO_FIELD_INFO(countLessonComplete) {
        info->description = ZH_WORDS_GETTER("stu.field.completedCount");
    }

    // 总课次
    DTO_FIELD(Int32, countLessonTotal);
    DTO_FIELD_INFO(countLessonTotal) {
        info->description = ZH_WORDS_GETTER("stu.field.totalCount");
    }
