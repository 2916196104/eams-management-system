...
PayFeesJsonVO::Wrapper CommonController::exePayFees(const PayFeesDTO::Wrapper& dto) {
	...
	//1.创建数据库
	try {
		YAML::Node config = YAML::LoadFile("D:/zero_one/zero-one-eams2603/eams-cpp/eams-c6-student/conf/data-source.yaml");
		std::string url = config["spring"]["datasource"]["url"].as<std::string>();
		std::string username = config["spring"]["datasource"]["username"].as<std::string>();
		std::string password = config["spring"]["datasource"]["password"].as<std::string>();
		int max_active = config["spring"]["datasource"]["druid"]["max-active"].as<int>();
		size_t start = url.find("//") + 2;
		size_t end = url.find("?");
		std::string address = url.substr(start, end - start);
		ConnPool pool(address, username, password, max_active);
		...
		if (affectedRows == 0) {
			...
			// 保留两位小数
			double tep = (int)((amount - paidAmount) * 100 + 0.5) / 100.0;
			std::string respondse = "Payment successful, the remaining amount to be paid is:" + std::to_string(tep);
	...
	// 捕获异常
		...
		vo->setStatus(RS_FAIL);
		vo->message = "Server error: " + std::string(e.what());
		return vo;
	}
}
