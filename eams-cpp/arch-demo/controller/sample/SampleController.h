...
#define API_TAG2 ZH_WORDS_GETTER("sample.tags.t2")
...
public:
	...
	API_DEF_ENDPOINT_INFO_AUTH(
		ZH_WORDS_GETTER("sample.query-by-id.summary"), querySampleById, SampleJsonVO::Wrapper, API_TAG,
	...
	// 3.1 定义导出接口描述
	API_DEF_ENDPOINT_INFO_QUERY_AUTH(ZH_WORDS_GETTER("sample.export.summary"), exportSample, SampleQuery, Void, API_TAG);
	// 3.2 定义导出接口处理
	API_HANDLER_ENDPOINT_OPTION_AUTH(API_M_GET, "/sample/export", exportSample, QUERIES(QueryParams, queryParams),
		API_HANDLER_QUERY_PARAM(query, SampleQuery, queryParams); return execExportSample(query););

	// 3.1 定义导出接口描述
	API_DEF_ENDPOINT_INFO_QUERY_AUTH(ZH_WORDS_GETTER("sample.export-pdf.summary"), exportSamplePdf, SampleQuery, StringJsonVO::Wrapper, API_TAG);
	// 3.2 定义导出接口处理
	API_HANDLER_ENDPOINT_OPTION_AUTH(API_M_GET, "/sample/export-pdf", exportSamplePdf, QUERIES(QueryParams, queryParams),
		API_HANDLER_QUERY_PARAM(query, SampleQuery, queryParams); API_HANDLER_RESP_VO(execExportSamplePdf(query)););

	// 3.1 定义导入接口描述
	API_DEF_ENDPOINT_INFO_FILE_AUTH(ZH_WORDS_GETTER("sample.import.summary"), importSample, SampleImportDTO::Wrapper, StringJsonVO::Wrapper, API_TAG);
	// 3.2 定义导入接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_POST, "/sample/import", importSample, REQUEST(std::shared_ptr<IncomingRequest>, request), execImportSample(request, authObject->getPayload()));

	// 3.1 定义新增接口描述
	API_DEF_ENDPOINT_INFO_AUTH(ZH_WORDS_GETTER("sample.post.summary"), addSample, StringJsonVO::Wrapper, API_TAG);
	// 3.2 定义新增接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_POST, "/sample", addSample, BODY_DTO(SampleAddDTO::Wrapper, dto), execAddSample(dto, authObject->getPayload()));

	// 3.1 定义修改接口描述
	API_DEF_ENDPOINT_INFO_AUTH(ZH_WORDS_GETTER("sample.put.summary"), modifySample, StringJsonVO::Wrapper, API_TAG);
	// 3.2 定义修改接口处理
	...
	// 3.1 定义删除接口描述
	API_DEF_ENDPOINT_INFO_AUTH(ZH_WORDS_GETTER("sample.delete.summary"), removeSample, ListJsonVO<String>::Wrapper, API_TAG);
	// 3.2 定义删除接口处理
	...
	API_DEF_ENDPOINT_INFO_AUTH(
		ZH_WORDS_GETTER("sample.query-one.summary"), queryOne, SampleJsonVO::Wrapper, API_TAG2,
		API_DEF_ADD_QUERY_PARAMS(String, "id", ZH_WORDS_GETTER("sample.field.id"), "d934050a8bb373e8f8eed0bf7507ec17", true);
	);
	...
	API_DEF_ENDPOINT_INFO_QUERY_AUTH(ZH_WORDS_GETTER("sample.query-all.summary"), queryAll, SampleQuery, SamplePageJsonVO::Wrapper, API_TAG2);
	// 3.2 定义测试声明式服务调用的接口1处理
...
private:
	...
	// 3.3 演示导出数据(PDF)
	StringJsonVO::Wrapper execExportSamplePdf(const SampleQuery::Wrapper& query);
	// 3.3 演示导入数据
	StringJsonVO::Wrapper execImportSample(std::shared_ptr<IncomingRequest> request, const PayloadDTO& payload);
	// 3.3 演示新增数据
