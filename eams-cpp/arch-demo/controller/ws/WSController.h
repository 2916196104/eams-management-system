...
public:
	ENDPOINT(API_M_GET, "chat", chat, REQUEST(std::shared_ptr<IncomingRequest>, request), API_HANDLER_AUTH_PARAME) {
		// 获取用户唯一ID
		auto id = authObject->getPayload().getId();
		// 设置提交的参数
		...
		response->putHeader(API_H_WS_TOKEN, request->getHeader(API_H_WS_TOKEN));
