...
void WSListener::readMessage(const WebSocket& socket, v_uint8 opcode, p_char8 data, oatpp::v_io_size size)
{
	...
	if (size == 0)
	{
		...
		auto pos = wholeMessage.find("::");
		if (pos != string::npos)
		{
			msgs.push_back(wholeMessage.substr(0, pos));
			msgs.push_back(wholeMessage.substr(pos + 2));
		}
		// 数据格式错误
		if (msgs.size() < 2)
		{
			socket.sendOneFrameText("message send fail, check your input!");
			return;
		}
		// 群发消息ID=all表示群发
		if ("all" == msgs[0])
