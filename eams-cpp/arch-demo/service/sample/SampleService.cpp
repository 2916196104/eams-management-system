...
bool SampleService::saveDataBatch(const oatpp::List<SampleAddDTO::Wrapper>& datas)
{
	UuidFacade uf(1);
	std::string ct = SimpleDateTimeFormat::format();
	std::list<SampleDO> dos;
	for (auto dto : *datas.get())
	{
		// 组装DO数据
		SampleDO& data = dos.emplace_back();
		ZO_STAR_DOMAIN_DTO_TO_DO(data, dto, Name, name, Sex, sex, Age, age);
		// 生成ID
		data.setId(uf.genUuid());
		// 设置创建人
		data.setCreateBy(dto->getPayload()->getUsername());
		// 设置创建时间
		data.setCreateTime(ct);
	}
	// 执行数据添加
	return SampleDAO().insertBatch(dos) != 0;
}
...
bool SampleService::removeData(const oatpp::List<oatpp::String>& ids)
{
	...
	int rows = dao.deleteByIds<SampleDO>(datas);
	if (rows == ids->size())
		return true;
	return false;
}
