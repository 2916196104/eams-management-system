...
public:		//定义接口
	...
	// 3.2 定义获取成绩列表接口处理
	API_HANDLER_ENDPOINT_OPTION_AUTH(API_M_GET, "org/gradetable/get-grade-list", queryPageScore, QUERIES(QueryParams, queryParams),
	...
	// 3.2 定义保存成绩接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_POST, "/org/gradetable/save-grade", saveScore, BODY_DTO(GradeManageDTO::Wrapper, dto), execSave(dto, authObject->getPayload()));

	// 3.1 定义删除成绩接口描述
	API_DEF_ENDPOINT_INFO_AUTH(ZH_WORDS_GETTER("Grade.delete.summary"), removeScore, ListJsonVO<String>::Wrapper, API_TAG);
	// 3.2 定义删除成绩接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_DEL, "org/gradetable/delete-grade", removeScore, BODY_DTO(List<String>, ids), execRemove(ids));
	...
	// 3.2 定义导入成绩接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_POST, "org/gradetable/import-grade", importScore, REQUEST(std::shared_ptr<IncomingRequest>, request), execImportScore(request, authObject->getPayload()));
