...
public:		//定义接口
	...
	// 定义保存公告接口端点
	API_HANDLER_ENDPOINT_AUTH(API_M_POST, "org/boardmanager/save-board", SaveBoard, BODY_DTO(BoardAddDTO::Wrapper, dto), executeAddBoard(dto, authObject->getPayload()));
	...
	API_HANDLER_ENDPOINT_AUTH(API_M_DEL, "org/boardmanager/remove-board", removeBoard, BODY_DTO(List<String>, ids), execRemoveBoard(ids));
	
	// 3.1 定义启用公告接口描述
	API_DEF_ENDPOINT_INFO_AUTH(ZH_WORDS_GETTER("board.start.summary"), startBoard, ListJsonVO<String>::Wrapper, API_TAG);
	// 3.2 定义启用公告接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_PUT, "org/boardmanager/start-board", startBoard, BODY_DTO(List<String>, ids), execStartBoard(ids));

	// 3.1 定义停用公告接口描述
	API_DEF_ENDPOINT_INFO_AUTH(ZH_WORDS_GETTER("board.end.summary"), endBoard, ListJsonVO<String>::Wrapper, API_TAG);
	// 3.2 定义停用公告接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_PUT, "org/boardmanager/end-board", endBoard, BODY_DTO(List<String>, ids), execEndBoard(ids));
