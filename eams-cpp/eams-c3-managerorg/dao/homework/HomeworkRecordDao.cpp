...
std::list<HomeworkDO> HomeworkRecordDAO::gethomeworklist(const GetHomeworkListQuery::Wrapper& query)
{
	...
	//// 执行查询
	//return sqlSession->executeQuery<HomeworkDO>(sql, HomeworkRecordMapper(), params);
	return{};
}

//获取作业详情，点击会传id，传回班级，作业标题，作业内容
PtrHomeworkDO HomeworkRecordDAO::gethomeworkdetail(std::string id)
{
	/*string sql = "SELECT class_id,title,content FROM homework WHERE `id`=?";
	return sqlSession->executeQueryOne<PtrHomeworkDO>(sql, PtrHomeworkRecordMapper(), "%s", id);*/
	return {};
}
