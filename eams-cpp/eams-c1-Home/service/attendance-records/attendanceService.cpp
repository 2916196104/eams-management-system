/*
 Copyright Zero One Star. All rights reserved.

 @Author: awei
 @Date: 2022/10/25 11:13:11

 Licensed under the Apache License, Version 2.0 (the "License");
 you may not use this file except in compliance with the License.
 You may obtain a copy of the License at

	  https://www.apache.org/licenses/LICENSE-2.0

 Unless required by applicable law or agreed to in writing, software
 distributed under the License is distributed on an "AS IS" BASIS,
 WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 See the License for the specific language governing permissions and
 limitations under the License.
*/
#include "stdafx.h"
#include "attendanceService.h"
#include "../../dao/attendance-records/attendanceDAO.h"
#include "id/UuidFacade.h"
#include "SimpleDateTimeFormat.h"
attendance_recordsPageDTO::Wrapper Lesson_StudentService::listAll(const attendance_recordsQuery::Wrapper& query)
{
	// 构建返回对象
	auto pages = attendance_recordsPageDTO::createShared();
	pages->pageIndex = query->pageIndex;
	pages->pageSize = query->pageSize;

	// 查询数据总条数
	Lesson_StudentDAO dao;
	uint64_t count = dao.count(query);
	if (count <= 0)
	{
		return pages;
	}

	// 分页查询数据
	pages->total = count;
	pages->calcPages();
	list<Lesson_StudentDO> result = dao.selectWithPage(query);
	// 将DO转换成DTO
	for (Lesson_StudentDO& sub : result)
	{
		auto dto = attendance_recordsDTO::createShared();
		ZO_STAR_DOMAIN_DO_TO_DTO(dto, sub, 
			decLessonCount, DecLessonCount,
			signTime,SignTime,
			signState,SignState,
			date,Date,
			startTime,StartTime,
			endTime,EndTime,
			courseName,CourseName,
			lessonTitle,LessonTitle,
			canEvaluate,CanEvaluate
			);
		pages->addData(dto);
	}
	return pages;
}

std::string Teach_EvaluationService::saveData(const attendance_recordsEvaluateDTO::Wrapper& dto)
{
	if (!dto)
	{
		return false;
	}

	// 直接使用 DO 对象（DAO.insert 接受 const Teach_EvaluationDO&）
	Ptrteach_evaluationDO data;

	// DTO -> DO 映射（根据实际 DTO 字段调整）
	ZO_STAR_DOMAIN_DTO_TO_DO_1(data,dto,
		LessonId,lesson_id,
		TeacherId, teacher_id,
		Score1,score1,
		Score2,score2,
		Score3,score3,
		Score4,score4,
		Content,content,
		AddTime, add_time,
		StudentId,student_id,
		Anonymity, anonymity
	);
	UuidFacade uf;
	data->setId(uf.genUuid());
	// 设置创建时间
	data->setAddTime(SimpleDateTimeFormat::format());
	// 执行数据添加
	Teach_EvaluationDAO dao;
	return dao.insert(data) == 1 ? data->getId() : "";
}