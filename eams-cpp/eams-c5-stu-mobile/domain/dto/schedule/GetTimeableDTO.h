...
class GetTimeableDTO : public oatpp::DTO
{
    ...
    DTO_FIELD(Int32, id);
    DTO_FIELD_INFO(id) {
        info->description = ZH_WORDS_GETTER("sample.field.id"); // 课程ID
    }

    // 课程标题
    DTO_FIELD(String, title);
    DTO_FIELD_INFO(title) {
        info->description = ZH_WORDS_GETTER("sample.field.title"); // 课程标题
    }

    // 课程序号
    DTO_FIELD(String, sn);
    DTO_FIELD_INFO(sn) {
        info->description = ZH_WORDS_GETTER("sample.field.sn"); // 课程序号
    }

    // 上课日期（保留原有）
    DTO_FIELD(String, date);
    DTO_FIELD_INFO(date) {
        info->description = ZH_WORDS_GETTER("sample.field.sex"); // 日期（若有专属key可替换，比如"sample.field.date"）
    }

    // 开始时间
    DTO_FIELD(String, start_time);
    DTO_FIELD_INFO(start_time) {
        info->description = ZH_WORDS_GETTER("sample.field.start_time"); // 开始时间
    }

    // 结束时间
    DTO_FIELD(String, end_time);
    DTO_FIELD_INFO(end_time) {
        info->description = ZH_WORDS_GETTER("sample.field.end_time"); // 结束时间
    }

    // 教师ID
    DTO_FIELD(Int32, teacher_id);
    DTO_FIELD_INFO(teacher_id) {
        info->description = ZH_WORDS_GETTER("sample.field.teacher_id"); // 教师ID
    }

    // 排课计划ID
    DTO_FIELD(Int32, schedule_id);
    DTO_FIELD_INFO(schedule_id) {
        info->description = ZH_WORDS_GETTER("sample.field.schedule_id"); // 排课计划ID
    }

    // 教师姓名
    DTO_FIELD(String, teacher_name);
    DTO_FIELD_INFO(teacher_name) {
        info->description = ZH_WORDS_GETTER("sample.field.teacher_name"); // 教师姓名
    }

    // 签到状态
    DTO_FIELD(Int32, sign_state);
    DTO_FIELD_INFO(sign_state) {
        info->description = ZH_WORDS_GETTER("sample.field.sign_state"); // 签到状态
    }
