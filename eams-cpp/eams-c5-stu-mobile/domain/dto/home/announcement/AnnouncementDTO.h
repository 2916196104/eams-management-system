...
class AnnouncementDTO : public oatpp::DTO
{
    ...
    DTO_FIELD(Int64, id);
    DTO_FIELD_INFO(id) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.id"); // 公告ID
    }

    // 2. 公告标题（varchar → String，对应DO的title）
    DTO_FIELD(String, title);
    DTO_FIELD_INFO(title) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.title"); // 公告标题
    }

    // 3. 公告类型（tinyint → Int32，对应DO的type）
    DTO_FIELD(Int32, type);
    DTO_FIELD_INFO(type) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.type"); // 公告类型
    }

    // 4. 公告内容（text → String，对应DO的content）
    DTO_FIELD(String, content);
    DTO_FIELD_INFO(content) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.content"); // 公告内容
    }

    // 5. 添加时间（datetime → String，对应DO的add_time）
    DTO_FIELD(String, add_time);
    DTO_FIELD_INFO(add_time) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.add_time"); // 添加时间
    }

    // 6. 编辑时间（datetime → String，对应DO的edit_time）
    DTO_FIELD(String, edit_time);
    DTO_FIELD_INFO(edit_time) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.edit_time"); // 编辑时间
    }
