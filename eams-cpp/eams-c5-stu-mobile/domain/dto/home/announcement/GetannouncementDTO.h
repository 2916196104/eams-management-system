...
class AnnouncementDTO : public oatpp::DTO
{
    ...
    DTO_FIELD_INFO(title) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.title");
    }

    // 3. 类型 学生端首页Banner  学生端首页提示  学生端首页弹窗（tinyint(1) → Boolean，DEFAULT '0',默认值1）
    DTO_FIELD(String, type);
    DTO_FIELD_INFO(type) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.content");
    }
    //4.发送内容(text)
    ...
    DTO_FIELD_INFO(cover) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.cover");
    }
    // 6. 创建人（bigint → Int64，DEFAULT NULL）
    DTO_FIELD(Int64, creator);
    DTO_FIELD_INFO(creator) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.creator");
    }
    // 7. 添加时间（datetime → String）
    DTO_FIELD(String, add_time);
    DTO_FIELD_INFO(add_time) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.add_time");
    }
    // 8. 删除标记（tinyint(1) → Boolean，DEFAULT '0'）
    DTO_FIELD(Boolean, deleted);
    DTO_FIELD_INFO(deleted) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.deleted");
    }
    // 9. 编辑时间（datetime → String）
    DTO_FIELD(String, edit_time);
    DTO_FIELD_INFO(edit_time) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.edit_time");
    }
    // 10. 编辑人（bigint → Int64，DEFAULT NULL）
    DTO_FIELD(Int64, editor);
    DTO_FIELD_INFO(editor) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.editor");
    }
    //11.状态（tinyint(1) → Boolean，DEFAULT '0'）
    DTO_FIELD(Boolean, state);
    DTO_FIELD_INFO(state) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.state");
    }
    //12.?int
    DTO_FIELD(Int64, sort_num);
    DTO_FIELD_INFO(sort_num) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.sort_num");
    }
    //13.唯一码varchar(50)
    DTO_FIELD(String, code);
    DTO_FIELD_INFO(code) {
        info->description = ZH_WORDS_GETTER("c5.home.Announcement.field.code");
    }
