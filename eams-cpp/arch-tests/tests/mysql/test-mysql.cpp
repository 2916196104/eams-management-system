...
TEST_F(MysqlTest, TestSS) {
	...
	// 创建表
	string table = "CREATE TABLE IF NOT EXISTS `lib_test` (`id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,`first` text NOT NULL,`last` text,`age` smallint(5) unsigned,PRIMARY KEY(`id`)) ENGINE = InnoDB DEFAULT CHARSET = utf8";
	...
	if (results.size() != 0) {
		...
		cout << sub.getId() << ":" << sub.getFirst() << "-" << sub.getLast() << "-" << sub.getAge() << endl;
	}
	cout << "query all:" << endl;
...
TEST_F(MysqlTest, TestTM) {
	...
	// 执行第一个DAO操作
	LibTest uObj;
	uObj.setFirst(ZH_WORDS_GETTER("mysql.zh7"));
	uObj.setLast(ZH_WORDS_GETTER("mysql.zh8"));
	int row = d1.update(uObj, "id = 1");
	// 执行第二个DAO操作
	row = row + d2.deleteByIds<LibTest>({ "2" });
	// 判断是否提交事务
	if (row == 2)
	{
		cout << "tm commit" << endl;
...
TEST_F(MysqlTest, TESTBATCH)
{
	// 创建测试数据
	std::list<LibTest> datas;
	for (int i = 0; i < 10; i++)
	{
		LibTest& uObj = datas.emplace_back();
		uObj.setFirst(std::string("first") + std::to_string(i));
		if (i % 2 == 0) uObj.setLast(std::string("last") + std::to_string(i));
		uObj.setAge(i);
	}

	// 批量插入数据
	LibTestDAO1 d1;
	int rows = d1.insertAutoPkBatch(datas, 6);
	ASSERT_EQ(rows, datas.size());
	...
	{
		uObj.setId(id++);
		uObj.setFirst(std::string("ufirst"));
	}
	// 批量修改
	rows = d1.updateBatch(datas);
	ASSERT_EQ(rows, datas.size());
}