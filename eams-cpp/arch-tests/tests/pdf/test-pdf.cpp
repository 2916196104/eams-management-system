...
TEST(PdfTest, TestTpl) {
	...
	PdfComponent::registerTplRender("test", [](YAML::Node* node, PdfComponent* pdf, void* realData) {
		// 创建一个页面
		auto page = pdf->getNewPage();

		// 绘制静态数据

		// 读取top数据
		auto top = (*node)["top"];
		pdf->drawTextCenterH(top["title"].as<std::string>(), pdf->getCurrPageHeight() - top["y"].as<HPDF_REAL>());

		// 读取bottom 数据
		auto bottom = (*node)["bottom"];
		pdf->drawTextCenterH(bottom["title"].as<std::string>(), bottom["y"].as<HPDF_REAL>());

		// 读取content数据
		auto content = (*node)["content"].as<std::vector<std::string>>();
		HPDF_REAL starty = 500;
		for (auto& item : content) {
			pdf->drawTextCenterH(item, starty);
			starty -= 20;
		}

		// 绘制动态数据
		std::string ctx = static_cast<std::string*>(realData)->c_str();
		pdf->drawTextCenterH(ctx, starty);
		});
