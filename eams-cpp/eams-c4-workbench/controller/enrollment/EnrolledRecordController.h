...
public:
	...
	ENDPOINT_INFO(EnrolledRecord) {
		...
		// 定义响应参数格式
		//API_DEF_ADD_RSP_JSON_WRAPPER(SamplePageJsonVO);根据情况在设置
		//定义查询参数
		API_DEF_ADD_PAGE_PARAMS();
	}
	// 3.2 定义获取报名记录接口处理
	ENDPOINT(API_M_GET, "/c4/workbench/enrollment/record", EnrolledRecord,QUERIES(QueryParams,queryEnrolled)) {
		//解析查询参数
	...
	// 3.2 定义获取报名记录详情接口处理
	ENDPOINT(API_M_GET, "/c4/workbench/enrollment/detail", EnrolledDetail, QUERIES(QueryParams, queryDetail)) {
		//解析查询参数
		API_HANDLER_QUERY_PARAM(DetailQuery, EnrolledQuery, queryDetail);
		// 响应结果
		API_HANDLER_RESP_VO(execEnrolledDetail(DetailQuery));
	}
private:
	//3.3 定义获取报名记录接口执行函数
	EnrolledRecordListJsonVO::Wrapper execQueryNameList(const EnrolledQuery::Wrapper& Record);
	//3.3 定义获取报名记录详情接口执行函数
	EnrolledRecordListJsonVO::Wrapper execEnrolledDetail(const EnrolledQuery::Wrapper& Detail);
};
