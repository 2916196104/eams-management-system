...
public:
	...
	//// 3.2 定义保存报名接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_POST, "/c4/workbench/enrollment/save", saveEnrollment, BODY_DTO(EnrollmentDTO::Wrapper, dto), execSaveEnrolled(dto, authObject->getPayload()));
private:
	//3.3 定义保存报名接口执行函数
	StringJsonVO::Wrapper execSaveEnrolled(const EnrollmentDTO::Wrapper& dto, const PayloadDTO& payload);
