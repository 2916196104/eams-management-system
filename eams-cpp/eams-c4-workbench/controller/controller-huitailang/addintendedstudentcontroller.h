...
public: // 定义接口
	...
	// 3.2 定义新增接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_POST, "/c4/addintendedstudnet", addstudent,
		BODY_DTO(intendedstudentDTO::Wrapper, dto), executeaddintendedstudent(dto));
private: // 定义接口执行函数 执行函数的返回值是响应给前端的json函数
	//这里返回值stringJsonVo 也是在vo中定义了相应的头文件
	intendedstudentJsonVO::Wrapper executeaddintendedstudent(const intendedstudentDTO::Wrapper& dto);
};
