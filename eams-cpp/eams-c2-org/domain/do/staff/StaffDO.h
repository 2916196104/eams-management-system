...
class StaffDO : public BaseDO
{
	...
	MYSQL_SYNTHESIZE(char, gender, Gender);     // 性别
	MYSQL_SYNTHESIZE(string, idCard, IdCard);  // 身份证号
	MYSQL_SYNTHESIZE(string, school, School);  // 毕业学校
	MYSQL_SYNTHESIZE(char, degree, Degree);     // 学历
	MYSQL_SYNTHESIZE(string, remark, Remark);  // 备注
	MYSQL_SYNTHESIZE(char, state, State);       // 在职状态
	MYSQL_SYNTHESIZE(char, isManager, IsManager); // 是否是管理者
	...
	MYSQL_SYNTHESIZE(char, deleted, Deleted);    // 逻辑删除标记
	...
	MYSQL_SYNTHESIZE(char, isInner, IsInner);    // 是否内部人员
...
public:
	...
	StaffDO() : BaseDO("staff")
	{
		...
		MYSQL_ADD_FIELD("gender", "c", gender);
		MYSQL_ADD_FIELD("id_card", "s", idCard);
		MYSQL_ADD_FIELD("school", "s", school);
		MYSQL_ADD_FIELD("degree", "c", degree);
		MYSQL_ADD_FIELD("remark", "s", remark);
		MYSQL_ADD_FIELD("state", "c", state);
		MYSQL_ADD_FIELD("is_manager", "c", isManager);
		...
		MYSQL_ADD_FIELD("is_inner", "c", isInner);
	}
};
