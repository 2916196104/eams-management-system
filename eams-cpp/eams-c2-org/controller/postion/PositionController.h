...
class PositionController : public oatpp::web::server::api::ApiController
{
...
public:
    ...
    ENDPOINT_INFO(getPositionDataPermissionList) {
        info->summary = "Get Position Data Permission List";
        info->description = "Query data permission list by position ID and pagination";
        info->addTag(API_TAG);
    }
    ENDPOINT("POST", "/positions/data/permissions/list", getPositionDataPermissionList,
        BODY_DTO(oatpp::Object<PositionDataPermissionQueryDTO>, request));

    // 批量删除职位
    ENDPOINT_INFO(deletePositions) {
        info->summary = "Batch Delete Positions";
        info->description = "Delete positions by IDs list";
        info->addTag(API_TAG);
    }
    ENDPOINT("POST", "/positions/delete", deletePositions,
        BODY_DTO(oatpp::Object<DeletePositionRequestDTO>, request));
