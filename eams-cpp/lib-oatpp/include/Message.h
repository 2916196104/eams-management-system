...
#else
#define RS_UNAUTHORIZED ResultStatus(u8"??¦Ä?????TOKEN???????", 401)
#define RS_FORBIDDEN ResultStatus(u8"?????????", 403)
#define RS_SERVER_ERROR ResultStatus(u8"??????????", 9994)
#define RS_PARAMS_INVALID ResultStatus(u8"?????????", 9995)
#define RS_CONTENT_TYPE_ERR ResultStatus(u8"ContentType????", 9996)
#define RS_API_UN_IMPL ResultStatus(u8"?????¦Ä???", 9997)
#define RS_SERVER_BUSY ResultStatus(u8"?????????", 9998)
#define RS_FAIL ResultStatus(u8"???????", 9999)
#define RS_SUCCESS ResultStatus(u8"???????")
#endif
