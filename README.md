...
- 负责模块：课程管理模块
- 核心工作：
  - 负责课程、科目、课程章节及关联课程等功能的后端开发，完成条件分页查询、详情查询、新增修改、批量删除、启用禁用等接口。
  - 基于 Spring Boot、MyBatis-Plus 和 MySQL 完成业务逻辑与数据持久化，并使用 DTO、VO、DO 分层组织接口参数和数据模型。
  - 实现课程图片、所属科目等关联数据的聚合查询，以及课程列表的 Excel 导出功能。
  - 针对关联课程业务增加课程存在性、重复关联和课程自关联校验，并通过事务保证批量写入和删除的数据一致性。
  - 使用 MyBatis-Plus 分页及条件查询，避免课程列表一次性加载全部数据；提供批量删除、批量启停能力，提高后台管理效率。
  - 通过统一 API 接口、参数校验、Swagger 接口文档和全局异常处理规范课程模块的开发与联调流程。
# `zero-one-eams`
